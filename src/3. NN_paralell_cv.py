...
def objective(trial):
    ...
    # --- 2. K-Fold CV の準備 ---
    K_SPLITS = 5  # 分割数 (通常は 5 か 10)
    kf = KFold(n_splits=K_SPLITS, shuffle=True, random_state=42)
    
    fold_scores = [] # 各Foldの検証RMSEを保存するリスト

    # --- 3. Foldごとのループ ---
    # X_train_scaled, Y_train_scaled は関数の外にある全体の学習データを使用
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_scaled)):
        ...
        # ★重要: Foldごとにモデルとオプティマイザを初期化（リセット）する
        ...
        for epoch in range(EPOCHS):
            ...
            for inputs, targets in train_loader:
                ...
                # LossがNaNになったら即終了（このパラメータは無効とする）
                if torch.isnan(loss):
                    return float('inf')
                loss.backward()
