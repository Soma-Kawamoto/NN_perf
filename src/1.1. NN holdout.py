...
【v8改: ホールドアウト検証 + GPU対応 + 共有DB対応 (Linux/WSL版)】
"""
import os
import numpy as np
import pandas as pd
...
USE_GPU_SETTING = config.getboolean('settings', 'USE_GPU', fallback=True) # ★追加: GPU設定
...
# plt.show() # WSLではGUIが出ない場合があるためコメントアウト推奨
...
if PERFORM_OPTUNA:
    ...
    study_name = (
        f"nn_holdout_({Bmtrain_min:.2f},{Bmtrain_max:.2f},{train_step:.2f})"
...
if not settings_match and PERFORM_TRAINING:
    ...
    for epoch in range(EPOCHS):
        ...
        epoch_loss = 0.0
        for inputs, targets in train_loader:
            # DataLoaderから取り出した際にも再度デバイスを明示（安全策）
            inputs, targets = inputs.to(device), targets.to(device)
            optimizer.zero_grad()
            outputs = model(inputs)
            loss = criterion(outputs, targets)
    ...
    plt.xlabel('Epoch'); plt.ylabel(f'Loss ({LossFunc})'); plt.grid(True, alpha=0.7); plt.legend()
    plt.savefig(os.path.join(plot_output_dir, "learning_curve.png"))

    # モデル保存（CPUに一度移してから保存するのがPyTorchのベストプラクティス）
    torch.save(model.to('cpu').state_dict(), model_weights_path)
    # 【重要】保存後、回帰（推論）を行うために再度デバイスに戻す
    model.to(device)
...
# plt.show() # WSL環境でGUIが出ない場合はコメントアウト
