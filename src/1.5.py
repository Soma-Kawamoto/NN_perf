...
akima_excel_path = os.path.join(
    base_dir, "2.Normal Magnetization Curve Extraction Folder", "2.Akima spline interpolation",
...
def calculate_acquisition_function(model, X, f_max, acq_type='UCB', kappa=2.5):
    ...
    if acq_type.upper() == 'UCB':
        return std  # UCB = κ * σ (κ=UCB_KAPPA)
...
def plot_marginal_acquisition(model, f_max, original_data_cache, title, next_points):
    ...
    bm_reg = 0.5  # 例として0.5Tを選択（必要に応じて調整可能）
    b_space_reg = np.linspace(-bm_reg, bm_reg, 100)
    X_reg = np.array([[bm_reg, b] for b in b_space_reg])
    acq_reg = calculate_acquisition_function(model, X_reg, f_max, ACQUISITION_FUNCTION, UCB_KAPPA)
