...
config_path = os.path.join(script_dir, "..", "config", "1.1.1. GPR Reg +-σ.ini")
config = configparser.ConfigParser()
config.read(config_path, encoding='utf-8')

ENABLE_PLOTTING = config.getboolean('settings', 'ENABLE_PLOTTING')
PERFORM_TRAINING = config.getboolean('settings', 'PERFORM_TRAINING')
USE_BAYESIAN_OPTIMIZATION = config.getboolean('settings', 'USE_BAYESIAN_OPTIMIZATION')
BO_ITERATIONS = config.getint('settings', 'BO_ITERATIONS')
ACQUISITION_FUNCTION = config.get('settings', 'ACQUISITION_FUNCTION')
UCB_KAPPA = config.getint('settings', 'UCB_KAPPA')
...
train_amp = list(np.round(np.arange(config.getfloat('settings', 'Bmtrain_min'), config.getfloat('settings', 'Bmtrain_max') + 1e-8, config.getfloat('settings', 'train_step')), 1))
...
akima_excel_path = os.path.join(
    ...
    f"Bm-Hb Curve_akima_{mat_name}_{target_freq}hz.xlsx"
)
...
def plot_acquisition_surface_3d(model, f_max, title):
    ...
    ax.set_xlabel('B [T]')
    ax.set_ylabel('Bm [T]')
    ax.set_zlabel(f'{ACQUISITION_FUNCTION} Value')
    plt.show()
    plt.close()
...
def plot_training_data_with_bo(initial_X, initial_Y, akima_points, bo_points_all, bo_points_current, title):
    ...
    if not df_loops.empty:
        ...
        for _, group_df in grouped_loops:
            ...
            label_added = True
    if not df_akima.empty:
        plt.scatter(df_akima['H'], df_akima['B'], s=80, c='red', marker='o', edgecolors='none', zorder=5, label='Akima Points')
...
def main():
    ...
    akima_H, akima_B = np.array([]), np.array([])
    if USE_AKIMA_DATA:
        ...
        try:
            ...
            akima_H, akima_B = df_akima_filtered['amp_Hb'].values, df_akima_filtered['amp_Bm'].values
            for Hb, Bm in zip(akima_H, akima_B):
                X_list.append([Bm, Bm]); Y_list.append([Hb])
                X_list.append([Bm, -Bm]); Y_list.append([-Hb])
        except FileNotFoundError:
            print(f"  -> 🔴 警告: Akimaデータファイルが見つかりません。: {akima_excel_path}")
        except Exception as e:
    ...
    if ENABLE_PLOTTING:
        ...
        akima_plot_points = {'H': np.concatenate([akima_H, -akima_H]), 'B': np.concatenate([akima_B, -akima_B])}
        plot_training_data_with_bo(
            X_train_initial, Y_train_initial,
            akima_plot_points, bo_points_all=[], bo_points_current=[],
    ...
    model.optimize_restarts(num_restarts=NUM_RESTARTS, max_iters=MAX_ITERS, optimizer=OPTIMIZER, verbose=True, messages=True)
    print("✅ 初期学習が完了しました。")
        ...
        for i in range(BO_ITERATIONS):
            ...
            for amp in train_amp:
                ...
                if df_search_space.empty:
                    print(f"  -> {amp:.1f}T ループ: 全ての候補点が既に追加済みのためスキップします。")
                    continue
                min_b, max_b = df_original['B'].min(), df_original['B'].max()
                if min_b == max_b: continue
                b_continuous_space = np.linspace(min_b, max_b, 200)
                ...
                ideal_b_idx = np.argmax(acq_values_continuous)
                b_ideal = b_continuous_space[ideal_b_idx]
            ...
            if new_points_added_count == 0:
                ...
                break
            if ENABLE_PLOTTING:
                plot_training_data_with_bo(X_train_initial, Y_train_initial, akima_plot_points, bo_added_points, current_iteration_points, f'Updated Training Data (Iteration {i+1})')
            ...
            print(f"  -> {new_points_added_count} 点をモデルに追加しました。モデルを更新・再学習します...")
    ...
    # ★★★ 修正点②：ENABLE_PLOTTINGがFalseでも必ず最終結果をプロット ★★★
    print("\n最終的な回帰結果をプロットします...")
    plt.show()
    plt.close()
