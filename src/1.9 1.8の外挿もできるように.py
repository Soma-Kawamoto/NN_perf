...
# Hc, Brファイルのパス設定
hc_br_base_dir = os.path.join(base_dir, "1.Training Data Folder", "assets")
...
if USE_AKIMA_DATA:
    ...
    target_regression_amps_akima = np.round(np.arange(Bmreg_min, Bmreg_max + 1e-8, step), 2)
    try:
        df_akima_full = pd.read_excel(akima_excel_path, sheet_name='Interpolated Data', engine='openpyxl')
        df_akima_filtered = df_akima_full[np.round(df_akima_full['amp_Bm'], 2).isin(target_regression_amps_akima)]
