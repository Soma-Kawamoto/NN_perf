...
def main():
    ...
    N_points = 1024
    HARMONIC_CUTOFF_ORDER = 512
    # -------------------------

    # ★★★ 修正箇所：相対パス設定 ★★★
    # ----------------------------------------------------------------------
    # 1. このスクリプト(.py)があるフォルダの場所を取得します。
    #    (GPR_perf フォルダのパスになります)
    base_dir = os.path.dirname(__file__)

    # 2. 入力と出力のベースパスを、上記 base_dir からの相対パスで定義します。
    base_input_root = os.path.join(base_dir,  "2.extracting data(xlsx)")
    base_output_root = os.path.join(base_dir, "3.Fourier Transform Correction")
    # ----------------------------------------------------------------------
