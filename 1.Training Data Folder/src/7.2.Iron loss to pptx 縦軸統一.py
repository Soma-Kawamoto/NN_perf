...
# --- 設定ファイルの読み込み ---
config_path = os.path.join(os.path.dirname(__file__), "..", "config", "7.2.Iron loss to pptx 縦軸統一_config.ini")
...
# 【変更点１】グラフ表示設定の追加
# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
# 線形フィット（赤の破線）をプロットするかどうか (True: 表示, False: 非表示)
PLOT_LINEAR_FIT = False

# --- パス設定 ---
MAT_NAME = config["settings"]["MAT_NAME"]
FREQ_LIST =  [int(f) for f in config["settings"]["FREQ_LIST"].split(",")]
AMP_START = float(config["settings"]["AMP_START"])
AMP_END = float(config["settings"]["AMP_END"])
...
INPUT_BASE_PATH = os.path.join(BASE_PATH, "assets", "3.Fourier Transform Correction")
OUTPUT_BASE_PATH = os.path.join(BASE_PATH, "assets", "8.Hysteresis Loss")
...
def find_input_file(directory, amp, freq):
    ...
    # ファイル名を生成して存在確認
    filename = f"Bm{amp_str}hys_{freq}hz.xlsx" # .xlsx を探す
...
def main():
    ...
    # PowerPointプレゼンテーションオブジェクトを作成
    prs = Presentation()
    prs.slide_width = Inches(13.333)  # Widescreen (16:9)
    prs.slide_height = Inches(7.5)
    
    excel_output_path = os.path.join(OUTPUT_BASE_PATH, f"Hysteresis_Analysis_Summary_縦軸統一_{MAT_NAME}_{FREQ_LIST}.xlsx")
    ...
    with pd.ExcelWriter(excel_output_path, engine='openpyxl') as writer:
        for amp in amp_list:
            ...
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            # 【変更点２】PLOT_LINEAR_FITの値に応じてプロットを分岐
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            if PLOT_LINEAR_FIT:
            ...
            slide_layout = prs.slide_layouts[6] # 「白紙」レイアウト
    ...
    output_ppt_path = os.path.join(OUTPUT_BASE_PATH, f"Hysteresis_Analysis_縦軸統一_{MAT_NAME}.pptx")
    try:
