...
def main():
    # --- 設定ファイルの読み込み ---
    config_path = os.path.join(os.path.dirname(__file__), "..", "config", "5.Downsampling_config.ini")
    config = configparser.ConfigParser()
    config.read(config_path, encoding='utf-8')

    mat_name = config["settings"]["mat_name"]
    amplitudes = np.round(np.arange(float(config["settings"]["amplitude_min"]), float(config["settings"]["amplitude_max"]) + 1e-8, float(config["settings"]["amplitude_step"])), 2)
    ...
    output_base = os.path.join(base_dir, "assets", "6.Downsampling")
    # ----------------------------------------------------------------------
