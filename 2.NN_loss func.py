    ...
    for i, amp in enumerate(pred_amps):
        ...
        if i < len(truth_data_blocks):
            ...
            if 'B' in df_truth_loop.columns and 'H_descending' in df_truth_loop.columns:
                # Bregとdf_truth_loop['B']の長さを揃える
                if len(Breg) != len(df_truth_loop['B'].values):
                    print(f"警告: Bm = {amp:.2f}T のBreg ({len(Breg)}点) と参照データ ({len(df_truth_loop['B'].values)}点) の長さが一致しません。リサンプリングを試みます。")
                    from scipy.interpolate import interp1d
                    f = interp1d(df_truth_loop['B'].values, df_truth_loop['H_descending'].values, kind='linear', fill_value="extrapolate")
                    h_true_desc = f(Breg)
                else:
                    h_true_desc = df_truth_loop['H_descending'].values
                b_true = df_truth_loop['B'].values
                if np.allclose(Breg, b_true, rtol=1e-5, atol=1e-2):
                    has_ref_data = True
                    Hb_pred = Hpred[-1]
                    ...
                    print(f"警告: Bm = {amp:.2f}T のBregと参照データの値が一致しません。RMSE計算をスキップします。")
            else:
