...
desktop_path = os.path.expanduser('~/Desktop')  # デスクトップパス
os.makedirs(model_dir, exist_ok=True)
os.makedirs(plot_output_dir, exist_ok=True)
os.makedirs(desktop_path, exist_ok=True)  # デスクトップフォルダの存在確認
...
def create_info_df(amp_value=None):
    info_data = {
        ...
        "値": [
            ...
            str(HIDDEN_LAYERS), str(ACTIVATION_FUNC), LEARNING_RATE, EPOCHS, BATCH_SIZE, GRAD_CLIP, "RMSELoss",
...
#plt.title(f'NN Regression vs Reference - {mat_name} {target_freq}Hz') # グラフにタイトルを追加
...
train_amp_set = set(np.round(train_amp, 2)) # 学習データの振幅をセットに変換して高速検索 (丸め誤差を考慮)
rmse_results = []
...
with torch.no_grad():
    for i, amp in enumerate(pred_amps):
        ...
        if num_points <= 1:
            print(f"警告: Bm = {amp:.2f}T の点数が1以下です。スキップします。")
            continue
        ...
        # 正解データとの比較
        # truth_data_blocksのインデックスとpred_ampsのインデックスが一致することを前提とする
        if i < len(truth_data_blocks):
            df_truth_loop = truth_data_blocks[i]
            if 'B' in df_truth_loop.columns and 'H_descending' in df_truth_loop.columns:
                # 正解データのB軸と生成したBregが一致するかチェック
                if np.allclose(Breg, df_truth_loop['B'].values, rtol=1e-5, atol=1e-2):
                    ...
                    comparison_sheets_data.append({'amp': amp, 'df': df_comp})
                     ...
                     print(f"警告: Bm = {amp:.2f}T のBregと参照データのB軸が一致しないため、RMSE計算と参照プロットをスキップします。")
            else:
                print(f"警告: Bm = {amp:.2f}T の参照データに 'B' または 'H_descending' 列がありません。")

        # 回帰結果のプロット色分け
        if amp in train_amp_set:
            plt.plot(Hpred, Breg, color='blue', linestyle='-', zorder=2, alpha=0.6) # 学習データに含まれる振幅は青色の透明度あり
        else:
            plt.plot(Hpred, Breg, color='red', linestyle='-', zorder=2) # 学習データに含まれない振幅は赤色
...
desktop_save_path_results = os.path.join(desktop_path, f"regression_results_{mat_name}_{target_freq}hz.svg")
plt.savefig(desktop_save_path_results, format='svg')
print(f"✅ 回帰結果のプロットをファイルに保存しました: {plot_save_path_results}")
print(f"✅ 回帰結果のSVGをデスクトップに保存しました: {desktop_save_path_results}")

# ★★★ 修正点：このコマンドを追加 ★★★
print("▶️ 回帰結果のプロットを表示します。このウィンドウを閉じると、次の処理に進みます...")
plt.show()

# --- 損失関数のプロット ---
# 'losses'リストが存在し、空でない場合のみプロットを実行（学習が実行された場合のみ）
if 'losses' in locals() and losses:
    print("\n損失関数の推移をプロットしています...")
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, len(losses) + 1), losses, linestyle='-', color='blue')
    plt.title(f'Loss vs Epoch - {mat_name} {target_freq}Hz (NN)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss (RMSE)')
    plt.yscale('log') # 変化が分かりやすいように対数スケールに
    plt.grid(True, which="both", ls="--")
    plt.tight_layout()
    
    plot_save_path_loss = os.path.join(plot_output_dir, f"loss_vs_epoch.png")
    plt.savefig(plot_save_path_loss)
    desktop_save_path_loss = os.path.join(desktop_path, f"loss_vs_epoch_{mat_name}_{target_freq}hz.svg")
    plt.savefig(desktop_save_path_loss, format='svg')
    
    print(f"✅ 損失関数のプロットをファイルに保存しました: {plot_save_path_loss}")
    print(f"✅ 損失関数のSVGをデスクトップに保存しました: {desktop_save_path_loss}")
    print("▶️ 損失関数のプロットを表示します...")
    plt.show()
# --- RMSE結果の強制出力 ---
if not rmse_results:
    print("\n" + "="*70)
    print("警告: RMSE結果が計算されませんでした。デフォルト値で出力します。")
    print("="*70)
    rmse_results = [{'Amplitude (T)': amp, 'RMSE (H_descending)': np.nan, 'Hb [A/m]': np.nan, 'RMSE/Hb': np.nan} for amp in pred_amps[:min(5, len(pred_amps))]]  # 最初の5つの振幅で仮データ
